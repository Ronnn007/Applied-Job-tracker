...
def get_sprint(offset=1):
    ...
    sprint_start = start_of_week - timedelta(weeks=offset)
...
def count_sprint_emails(start_date, label, end_date=None):
    # Defining sprint 
    query = (f'label:{label} after:{start_date}')
...
def run():
    ...
    query_date, sprint_end =  get_sprint(offset=0) # Define Sprint First!
    
    #   For this date, update -  Job applications:
    applications = count_sprint_emails(start_date=query_date, label="Job Applications", end_date=sprint_end)

    #   For this date, update -  Job rejections:
    rejections = count_sprint_emails(start_date=query_date, label="Job Rejections", end_date=sprint_end)
...
>>>>>>> 9a5d619631bef16f0c568ca84edfed60f7c87450
    run()